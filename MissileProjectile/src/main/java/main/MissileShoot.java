	...
	public void launchedMissile(ProjectileLaunchEvent e) {
		...
		for (Entity entity : entities) {
			...
			Vector player2target_vec = new Vector(
					...
					living_entity.getEyeLocation().getY() - p.getEyeLocation().getY(),
...
//			}
			...
			System.out.println(living_entity.getName() + " : " + distance);
		...
		System.out.println(" -> " + target.getName() + " : " + minDistance);
		...
		missileVec = missileVec.multiply(prj.getVelocity().length()); // 자동추격 발사체의 힘을 원래 발사체의 힘으로 수정
